...
pub fn execute_code(params: &Value) -> RpcResponse {
    ...
    let (cmd, ext) = match language {
        ...
        "node" | "javascript" | "js" => {
            return RpcResponse::error(
                "node/javascript execution disabled — requires `unsafe-rpc-exec` feature"
                    .to_string(),
            );
        }
        other => {
    ...
    let tmp_path =
        std::env::temp_dir().join(format!("canal_exec_{}_{counter}.{ext}", std::process::id()));
    {
...
mod tests {
    ...
    fn test_temp_file_permissions() {
        ...
        let tmp_path = std::env::temp_dir().join(format!("canal_exec_{}.sh", std::process::id()));
