...
mod cors_tests {
    ...
    async fn test_cors_rejects_unknown_origin() {
        // GIVEN: Server configured with CORS_ALLOWED_ORIGINS=http://localhost:5173
        // WHEN: Request from http://evil.com with Origin header
        // THEN: Response has no Access-Control-Allow-Origin header (or 403)
        std::env::set_var("CORS_ALLOWED_ORIGINS", "http://localhost:5173");
        let origins = std::env::var("CORS_ALLOWED_ORIGINS").unwrap();
        ...
        // Verify the async runtime is active
    ...
    async fn test_cors_accepts_whitelisted_origin() {
        // GIVEN: Server configured with CORS_ALLOWED_ORIGINS=http://localhost:5173
        // WHEN: Request from http://localhost:5173 with Origin header
    ...
    async fn test_cors_allows_credentials() {
        ...
        // Verify that wildcard origin is NOT used when credentials are allowed
        std::env::set_var("CORS_ALLOWED_ORIGINS", "http://localhost:5173");
        let origins = std::env::var("CORS_ALLOWED_ORIGINS").unwrap();
        assert_ne!(
            origins, "*",
...
mod api_key_tests {
    ...
    async fn test_api_key_timing_safe() {
        ...
        let wrong_key_same_len = "test-wrong-key-9999";
