...
//!
//! Connects to Premiere Pro via ExtendScript/CEP Panel.
//! Requires a companion CEP panel to be installed.
...
/// Adobe Premiere Pro adapter using ExtendScript
pub struct PremiereAdapter {
    capabilities: AdapterCapabilities,
    connected: bool,
}

impl PremiereAdapter {
    pub fn new() -> Self {
        Self {
    ...
    fn build_capabilities() -> AdapterCapabilities {
        ...
        color_ops.insert("lumetri_adjust".to_string());

        let mut audio_ops = HashSet::new();
        audio_ops.insert("normalize_audio".to_string());
        audio_ops.insert("apply_audio_effect".to_string());
        ...
        AdapterCapabilities {
            application: Application::AdobePremiere,
            version: None,
            ...
            supports_remote_api: false, // CEP is local only
...
impl Default for PremiereAdapter {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl CreativeAdapter for PremiereAdapter {
    ...
    async fn is_running(&self) -> bool {
        ...
        #[cfg(target_os = "windows")]
        {
            if let Ok(output) = tokio::process::Command::new("tasklist")
                .args(["/FI", "IMAGENAME eq Adobe Premiere Pro.exe"])
                .output()
                .await
            {
                return String::from_utf8_lossy(&output.stdout).contains("Premiere Pro");
            }
        }
    ...
    async fn connect(&mut self) -> Result<(), AdapterError> {
        if !self.is_running().await {
            return Err(AdapterError::NotRunning(
                "Adobe Premiere Pro is not running".to_string(),
            ));
        }
        self.connected = true;
        Ok(())
    ...
    async fn get_project_info(&self) -> Result<ProjectInfo, AdapterError> {
        Err(AdapterError::NotSupported(
            "get_project_info not yet implemented for Premiere".to_string(),
        ))
    }

    async fn get_timelines(&self) -> Result<Vec<TimelineInfo>, AdapterError> {
        Err(AdapterError::NotSupported(
            "get_timelines not yet implemented for Premiere".to_string(),
        ))
    }

    async fn get_active_timeline(&self) -> Result<TimelineInfo, AdapterError> {
        Err(AdapterError::NotSupported(
            "get_active_timeline not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn get_clips(
        ...
        Err(AdapterError::NotSupported(
            "get_clips not yet implemented for Premiere".to_string(),
        ))
    }

    async fn get_clip(&self, _clip_id: &ClipId) -> Result<ClipInfo, AdapterError> {
        Err(AdapterError::NotSupported(
            "get_clip not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn add_clip(
        ...
        Err(AdapterError::NotSupported(
            "add_clip not yet implemented for Premiere".to_string(),
        ))
    }

    async fn delete_clip(&self, _clip_id: &ClipId) -> Result<(), AdapterError> {
        Err(AdapterError::NotSupported(
            "delete_clip not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn trim_clip(
        ...
        Err(AdapterError::NotSupported(
            "trim_clip not yet implemented for Premiere".to_string(),
        ))
    }

    async fn get_color_nodes(&self, _clip_id: &ClipId) -> Result<Vec<NodeId>, AdapterError> {
        // Premiere doesn't have nodes, return empty
        Ok(vec![])
    }
    ...
    async fn add_color_node(
        ...
        // Premiere uses Lumetri instead of nodes
        Err(AdapterError::NotSupported(
            "Premiere uses Lumetri panel instead of color nodes".to_string(),
        ))
    }

    async fn apply_lut(&self, _clip_id: &ClipId, _lut_path: &str) -> Result<(), AdapterError> {
        Err(AdapterError::NotSupported(
            "apply_lut not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn apply_color_wheels(
        ...
        Err(AdapterError::NotSupported(
            "apply_color_wheels not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn apply_primaries(
        ...
        Err(AdapterError::NotSupported(
            "apply_primaries not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn apply_curves(
        ...
        Err(AdapterError::NotSupported(
            "apply_curves not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn copy_grade(
        ...
        Err(AdapterError::NotSupported(
            "copy_grade not yet implemented for Premiere".to_string(),
        ))
    }

    async fn analyze_audio(&self, _clip_id: &ClipId) -> Result<AudioAnalysis, AdapterError> {
        Err(AdapterError::NotSupported(
            "analyze_audio not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn normalize_audio(
        ...
        Err(AdapterError::NotSupported(
            "normalize_audio not yet implemented for Premiere".to_string(),
        ))
    }

    async fn apply_audio_gain(&self, _clip_id: &ClipId, _gain_db: f32) -> Result<(), AdapterError> {
        Err(AdapterError::NotSupported(
            "apply_audio_gain not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn export(
        ...
        Err(AdapterError::NotSupported(
            "export not yet implemented for Premiere".to_string(),
        ))
    }

    async fn get_export_progress(&self) -> Result<f32, AdapterError> {
        Err(AdapterError::NotSupported(
            "get_export_progress not yet implemented for Premiere".to_string(),
        ))
    }

    async fn cancel_export(&self) -> Result<(), AdapterError> {
        Err(AdapterError::NotSupported(
            "cancel_export not yet implemented for Premiere".to_string(),
        ))
    }

    async fn add_marker(&self, _clip_id: &ClipId, _marker: &Marker) -> Result<String, AdapterError> {
        Err(AdapterError::NotSupported(
            "add_marker not yet implemented for Premiere".to_string(),
        ))
    }

    async fn get_markers(&self, _clip_id: &ClipId) -> Result<Vec<Marker>, AdapterError> {
        Err(AdapterError::NotSupported(
            "get_markers not yet implemented for Premiere".to_string(),
        ))
    }

    async fn delete_marker(&self, _marker_id: &str) -> Result<(), AdapterError> {
        Err(AdapterError::NotSupported(
            "delete_marker not yet implemented for Premiere".to_string(),
        ))
    }
    ...
    async fn execute_operation(
        ...
        Err(AdapterError::NotSupported(format!(
            "Operation '{}' not yet implemented for Premiere",
            operation
        )))
    }
}
