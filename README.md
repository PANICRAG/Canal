<p align="center">
  ...
  <p align="center">
    Unified AI Runtime.<br>
    Provider-agnostic inference gateway built on Rust and Tokio.
  </p>
</p>
...
Multi-provider LLM routing, deterministic agent orchestration, and native MCP integration. Canal abstracts 6+ LLM providers behind a single **OpenAI-compatible API** — no application code changes required.

---

## Six Layers. One Binary.

Canal is structured as six composable layers:

| Layer | Name | What it does |
|-------|------|-------------|
| **L06** | API Surface | Axum HTTP server. REST endpoints, SSE streaming, bearer auth, rate limiting. |
| **L05** | Orchestration | StateGraph execution engine. Swarm (parallel dispatch), Plan-Execute (sequential decomposition), Expert (specialist routing with judge aggregation). |
| **L04** | MCP Gateway | Native Model Context Protocol. Tool discovery, schema aggregation, server-side execution. Client and server mode. |
| **L03** | Tool Execution | Sandboxed runtime with filesystem access controls. Plugin system with connector framework. Per-tool permission boundaries. |
| **L02** | Agent Core | Agentic loop with composable skill registry. Session persistence, context windowing, constraint profiles. Budget enforcement and stall detection. |
| **L01** | LLM Routing | Provider-agnostic request routing with automatic failover, cost-aware model selection, load balancing, and semantic response caching. |

## Drop-in API Compatibility

Canal implements the OpenAI chat completions API. Redirect your existing client library — multi-provider routing, response caching, and execution tracing are handled transparently.
...
**Supported providers:** Anthropic Claude, OpenAI GPT, Google Gemini, Alibaba Qwen, OpenRouter, Ollama (local).
...
Deterministic multi-agent execution via directed acyclic graphs. Three coordination topologies:
- **Swarm** — Parallel dispatch with context handoff
- **Plan-Execute** — Decompose then execute sequentially
- **Expert** — Specialist routing with judge aggregation
...
Three-layer prompt constraint model:
- **Hard constraints** — Output format, security boundaries
- **Soft constraints** — Role anchoring, reasoning mode
- **User preferences** — Custom instructions, runtime-editable without redeployment
